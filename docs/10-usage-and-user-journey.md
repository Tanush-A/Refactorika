...
```
$ editmemory confirm --variant result-type
```
...
$ editmemory context
```
- Generates `.editmemory/context/<module>.md` files summarizing the now-canonical convention per module.

Each step is also runnable standalone against a diff (`editmemory check <diff>`) for CI/pre-commit use, independent of the full run.
