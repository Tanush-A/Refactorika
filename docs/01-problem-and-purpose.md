...
This problem is getting more urgent, not less: a growing share of production code today is AI-generated, written quickly and inconsistently across sessions, without a unifying convention. That's "AI slop" — code that works but accumulates inconsistency invisibly, and isn't fit to ship or maintain as-is. Refactorika exists to make that code safe to bring back under one convention before it goes further into production.

## Solution

**Refactorika** is a convention-audit and guided-refactor layer designed to be run *as a plugin* against a codebase that's ready for refactor. Instead of trusting an agent to refactor unsupervised, or doing the work by hand, a team runs Refactorika on the repo and it:
