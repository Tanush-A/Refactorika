...
- **It DOES:** Audits a TypeScript repo for error-handling inconsistency, proposes the dominant convention (human confirms), plans a dependency-safe fix order, then **guides an agent through the refactor file-by-file with verification gates** (parse → lint → `tsc` → tests → call-site sweep) so **no edit lands in a broken state** — and persists the result as context files.
...
- **The victim:** A dev/team with a legacy or AI-slop TypeScript codebase (inconsistent error handling across files, written fast across many agent sessions) who wants the mechanical refactor done *safely*, not by hand and not by trusting an agent blind.
- **The 30-second magic moment:** Run audit on a deliberately inconsistent repo → confirm dominant pattern → watch files get fixed live → a planted compile-clean-but-behavior-breaking edit gets **caught by the test gate, rolled back, and the agent recovers** via the re-propose loop → token chart shows a fraction of the baseline.
...
- **Fake what we can't build.** The demo repo is **curated**: deliberate, known inconsistencies; planted violations; a planted compile-clean-but-test-breaking edit; explicit return-type annotations (tree-sitter sees syntax, not inferred types — see Risks). Ground truth is known so precision/recall is honest.
- **Reliability over code quality.** Fewer moving parts. Hardcoded fallback for every external call (Redis → local JSON; Fetch network → direct CLI/MCP; tsc/tests → skip-and-record, never silent-pass).
...
- **One language:** TypeScript only.
- **Detection is tree-sitter-only** → only *syntactically visible* types: **explicitly-annotated** return types + recognized `Result`/`Either` type names. Inferred types and cross-file alias resolution are **out** (honest blind spot, §10). `Promise<X>` is unwrapped before classifying.
- **Call sites are best-effort, not IDE-grade:** direct `import` + direct `call_expression` only. Dynamic dispatch, re-exports/barrels, string-keyed access, cross-language = known false negatives, framed honestly. False-negative rate comes from the **ground-truth eval (§7)**, NOT from Sentry.
- **Sentinel caution:** `T | null` is often a legit "not found," not an error — counted as `sentinel` only with a corroborating signal, else reported `ambiguous` and **not** a deviation (don't inflate the inconsistency number).

## Stack & conventions
- **Language:** TypeScript (analysis target + likely impl). Fetch.ai wrapper layer is **Python** (uAgents) — see Delivery.
- **Parsing/analysis:** `tree-sitter-typescript` (audit + pre-edit parse gate) · `tsc --noEmit` (post-edit typecheck gate, single-file scope where possible) · ESLint/Prettier (lint gate) · repo test command (behavioral gate) · AST symbol search + grep fallback (call-site detection).
...
- **Run / lint / test:** TBD — pin the demo-repo `tsconfig.json` + a fast, deterministic test suite early (the gates depend on them).
...
- **Fetch.ai / ASI:One orchestration** (sponsor delivery — verified feasible). ASI:One is a **hosted orchestrator LLM** (you don't deploy it; OpenAI-compatible API at `api.asi1.ai/v1`, `model="asi1"`); it discovers and calls a custom agent over the **Chat Protocol**. Refactorika **must run locally** (needs filesystem + `tsc` subprocess) — Agentverse-hosted agents are sandboxed (no `subprocess`/`os`/`tree-sitter`). So:
  ...
  - Long tasks: ASI:One uses a **poll** model (no push/callback) — build loading states. Run `tsc`/tests via async subprocess so they don't block the agent event loop.
...
2. **Lint/format** — ESLint/Prettier on touched files; reject *new* violations only (not pre-existing).
3. **Type check** — `tsc --noEmit`; fail → roll back. No edit committed non-compiling.
4. **Behavioral test gate** — run tests covering touched files; compiles ≠ behaves (a `throw`→`Result<T>` conversion can regress silently). Roll back on fail; record a **skip** where no test covers the file.
5. **Call-site sweep + handled-result check** — re-scan recorded call sites: (a) none left in old convention, (b) callers actually *consume* the new convention (`Result.ok` checked, errors not silently dropped).
...
5. **Plant a compile-clean but test-breaking edit** → test gate catches it after `tsc` passes → rollback to checkpoint → agent recovers.
...
- **Hour 0 (all together, ~1h):** freeze the tool signatures + per-edit log schema above. Pin the demo-repo `tsconfig` + test command.
...
| **2 — Audit** | §5.1 tree-sitter classification (3 variants + mixed/ambiguous, Promise-unwrap) · `confirm_convention` · LangCache keying | Most open-ended → timebox hardest, descope first. |
...
| **4 — Verify + safety** | §5.5 harness (parse→lint→tsc→tests→sweep, re-propose, escalation) · §5.7 checkpoints/atomic units/blast-radius/re-audit · Redis Agent Memory · Sentry [Reach] | |
...
- Inferred / imported-type resolution via the TypeScript compiler API (would close the tree-sitter blind spot).
